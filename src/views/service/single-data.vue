              ...
              <h2 class="hc-x">Buy Data</h2>
                ...
                <form @submit.prevent="handleSubmit">
                     <div class="ml-xf">
                        ...
                        <select name="" id="" placeholder="Chosse Network" v-model="network" @blur="getIndex(network.networkid)">
                           <option :value="{networkid:'1',networkname:'MTN'}">MTN</option>
                           <option :value="{networkid:'2',networkname:'AIRTEL'}">Airtel</option>
                           <option :value="{networkid:'3',networkname:'9MOBILE'}">9mobile</option>
                    ...
                    <div class="ml-xf">
                        ...
                        <select name="" id="" placeholder="Chosse Network" v-model="plan">
                            <option :value="{id :item.id,amount:item.price,name:item.name,plan:item.plan}" v-for="item in plans" :key="item.id" v-show="network!=1">{{item.name}} {{item.plan}} &#8358;{{item.price}}</option>
                     ...
                     <div class="ml-xf">
                        <label for="email">Phone Number</label> <br>
                         <input type="number" placeholder="08132112841" v-model="phone" required maxlength="11" minlength="11"
    ...
    methods: {
        ...
        async  getIndex(index){
              try{
                ...
                const mtnapi = await  axios.get(`${process.env.VUE_APP_BASE_URL}api/getmanagement`,
                {
                headers: {
          ...
          if(index==1 && this.mtnapi == 1 ){
           ...
           const data  = JSON.parse(localStorage.getItem('user'));
          this.token = data.data.token
        const plan = await  axios.get(`${process.env.VUE_APP_BASE_URL}api/smeplans?type=data`,
        {
               headers: {
                    Authorization:"Bearer "+ this.token
                }
        ...
        const network =  await  axios.get(`${process.env.VUE_APP_BASE_URL}api/smeplans?type=network`,
        {
               headers: {
                    Authorization:"Bearer "+ this.token
                }
