                ...
                <form @submit.prevent="handleSubmit">
                    ...
                    <div class="ml-xf">
                        ...
                        <select v-model="service" @blur="getVariation()" >
                            <option :value="{ename:item.name,service:item.serviceID}" v-for="item in servicesIDs" :key="item.name">
                     ...
                     <div class="ml-xf">
                        ...
                        <select v-model="variation_code" placeholder="Choose a Payment method">
                            ...
                            <option :value="item.variation_code" v-for="item in variations" :key="item.variation_code">{{item.name}}</option>
                        </select>
                    </div>
                     <div class="ml-xf">
                        <label for="meter">Meter Number</label> <br>
                         <input type="number" placeholder="123456789" v-model="billerCode" required  @keyup="verifyNumber" step="any">
                    </div>
                    <span style="color:crimson;font-weight:800;font-size:1rem;text-align:center;padding-left:20px" v-show="showwarn">Please choose a provider first</span>

                     <div class="ml-xf">
                        <label for="Name">Name </label> <br>
                         <input type="name" placeholder="123456789" v-model="name" required  disabled>
                    </div>
                     <div class="ml-xf">
                        <label for="meter">Address</label> <br>
                         <input type="name" placeholder="123456789" v-model="address" required disabled >
                    </div>
                     <div class="ml-xf">
                        <label for="meter">Amount</label> <br>
                         <input type="number" placeholder="0.00" v-model="amount" required step="any" >
                    </div>
                    
                     
                    <div class="ml-xf">
                      <button :disabled="isDisabled" :class="isDisabled?'disableClass' :'activeClass'" >{{btnText}}</button>
                    </div>
...
export default {
    ...
    methods: {
          ...
          async getVariation(){
            ...
            this.token = datas.data.token
                ...
                try{
                const response  = await axios.get(`${process.env.VUE_APP_BASE_URL}api/getcableorbillplan?serviceid=`+this.service.service,{
                     headers:headers
                }
          ...
          async verifyNumber(){
            ...
            try{
                 const verify = await axios.post(`${process.env.VUE_APP_BASE_URL}api/getmearchant`,data,
                    {
                        headers: headers
                    }
                 )
        ...
        if(parseInt(this.amount)<=parseInt(this.balance)){
           if(this.amount >=500){
            ...
            this.status=true,
            this.message='Transaction generated successfully'
             this.setTimeout = setTimeout(()=>{
              ...
              this.status = false,
            this.message= 'Minimum meter token is 500'
  ...
  async mounted(){
        try{
            const getService = await axios.get('https://vtpass.com/api/services?identifier=electricity-bill')
